package slackget10::Network::Connection::FILE;
...
sub __get_file {
	...
	{
		$file = slackget10::File->new($self->strip_slash($self->path().'/'.$remote_file), 'binary' => 1);
	}
	else
	{
		$file = slackget10::File->new($self->strip_slash($self->path().'/'.$remote_file), 'binary' => 0);
	}
...
1; # End of slackget10::Network::Connection::FILE
