...
# 	print Data::Dumper::Dumper($xml);
	...
	{
		warn "[slackget10::Media] no update-repository found for the update source '$self->{ID}'\n";
		return undef;
	}
	return 1;
}
...
sub next_host
{
	...
	{
		warn "[slackget10::Media] no more host in the 'fast' category for update source '$self->{ID}'\n";
		...
		{
			warn "[slackget10::Media] no more host in the 'slow' category for update source '$self->{ID}'\n";
			return undef;
		}
	}
	return $self->host ;
